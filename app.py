...
def load_model():
    return joblib.load('model/xgb_cfd_model.pkl')
model = load_model()

# --- Expected input columns ---
INPUT_COLUMNS = [
    'y_U1','y_U2','y_U3','y_U4','y_U5','y_U6','y_U7','y_U8',
    'y_L1','y_L2','y_L3','y_L4','y_L5','y_L6','y_L7','y_L8',
    'alpha','Mach'
]

# --- Validation function with 7 explicit checks ---
def validate_inputs(df):
    """
    Perform 7 validation checks on the input DataFrame.
    ...
    if missing.any():
        cols = [col for col in df.columns if missing[col]]
        errors.append(f"❌ Missing values found in columns: {', '.join(cols)}")

    # 2. Non-numeric values
    non_numeric = [col for col in df.columns if not pd.api.types.is_numeric_dtype(df[col])]
    if non_numeric:
        errors.append(f"❌ Non-numeric values found in columns: {', '.join(non_numeric)}")

    # 3. Wrong column count
    if df.shape[1] != len(INPUT_COLUMNS):
        errors.append(f"❌ Wrong number of columns: expected {len(INPUT_COLUMNS)}, found {df.shape[1]}")

    # Only continue if columns are correct
    if set(df.columns) != set(INPUT_COLUMNS):
        # If columns are wrong, skip other checks
        return False, errors, warnings

    # 4. Physical violation: upper < lower
    y_U_cols = [f'y_U{i}' for i in range(1,9)]
    y_L_cols = [f'y_L{i}' for i in range(1,9)]
    for idx, row in df.iterrows():
        y_U = row[y_U_cols].values
        y_L = row[y_L_cols].values
        if np.any(y_U < y_L):
            errors.append(f"❌ Row {idx+1}: Physical violation: upper surface below lower surface (y_U < y_L)")
            break

    # 5. Negative Mach number
    if (df['Mach'] < 0).any():
        bad_idx = df.index[df['Mach'] < 0][0]
        errors.append(f"❌ Row {bad_idx+1}: Negative Mach number detected (Mach={df.loc[bad_idx, 'Mach']})")

    # 6. Extreme angle of attack (warning)
    alpha_min = df['alpha'].min()
    alpha_max = df['alpha'].max()
    if alpha_min < -2 or alpha_max > 14.5:
        warnings.append(f"⚠️ Angle of attack (alpha) outside training range [-2°, 14.5°]. Found: [{alpha_min:.2f}°, {alpha_max:.2f}°]. Predictions may be unreliable.")

    # 7. Valid control data (all values finite, not NaN/inf, and reasonable geometry)
    # (a) Infinite values
    if np.isinf(df.select_dtypes(include=[np.number]).values).any():
        errors.append("❌ Infinite values detected in input data.")
    # (b) Thickness everywhere positive (should be, if upper > lower, but double-check for degenerate geometry)
    for idx, row in df.iterrows():
        thickness = row[y_U_cols].values - row[y_L_cols].values
        if np.any(thickness <= 0):
            errors.append(f"❌ Row {idx+1}: Zero or negative thickness at some chordwise points.")
            break
        if np.max(thickness) < 0.001:
            warnings.append(f"⚠️ Row {idx+1}: Very thin airfoil (max thickness < 0.001). Results may be unreliable.")
...
if uploaded_file is not None:
    ...
    # Try to set columns if possible
    if df.shape[1] == len(INPUT_COLUMNS):
        df.columns = INPUT_COLUMNS
    # Validate
    is_valid, errors, warnings = validate_inputs(df)
    ...
    for warn in warnings:
        st.warning(warn)
    if is_valid:
        st.success("✅ All validation checks passed! Ready for prediction.")
    else:
    ...
    if is_valid:
        ...
        y_U_cols = [f'y_U{i}' for i in range(1,9)]
        y_L_cols = [f'y_L{i}' for i in range(1,9)]
        y_U_vals = df[y_U_cols].values
        y_L_vals = df[y_L_cols].values
        camber = (y_U_vals + y_L_vals)/2
        thickness = y_U_vals - y_L_vals
        df['camber_mean'] = camber.mean(axis=1)
        df['max_camber'] = camber.max(axis=1)
        df['max_camber_idx'] = np.argmax(camber, axis=1)
        df['thickness_mean'] = thickness.mean(axis=1)
        df['max_thickness'] = thickness.max(axis=1)
        df['max_thickness_idx'] = np.argmax(thickness, axis=1)

        if st.button("Predict"):
            feature_cols = INPUT_COLUMNS + [
                'camber_mean', 'max_camber_idx', 'max_camber',
                'thickness_mean', 'max_thickness', 'max_thickness_idx'
            ]
